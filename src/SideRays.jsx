...
import SideRaysContext from './SideRaysContext';
import './SideRays.css';
...
const SideRays = ({
  ...
  // mouse tilt — stored as refs, never triggers re-render
  const targetTiltRef   = useRef(tilt);
  const currentTiltRef  = useRef(tilt);
  const mouseTiltRafRef = useRef(null);
  ...
  // Mouse tracking — writes directly to uniform, no setState → no re-render
  useEffect(() => {
    if (!followMouse) return;

    const onMove = e => {
      const nx = (e.clientX / window.innerWidth) * 2 - 1;
      targetTiltRef.current = -nx * mouseTiltRange;
    };
    window.addEventListener('mousemove', onMove);

    const loop = () => {
      currentTiltRef.current += (targetTiltRef.current - currentTiltRef.current) * 0.025;
      if (uniformsRef.current) {
        uniformsRef.current.iTilt.value = currentTiltRef.current;
      }
      // Broadcast current tilt to all context subscribers
      if (publishRef?.current) publishRef.current(currentTiltRef.current);
      mouseTiltRafRef.current = requestAnimationFrame(loop);
    };
    mouseTiltRafRef.current = requestAnimationFrame(loop);

    return () => {
      window.removeEventListener('mousemove', onMove);
      cancelAnimationFrame(mouseTiltRafRef.current);
    };
...
void main() {
  ...
  gl_FragColor = color;
      ...
      const loop = t => {
        ...
        // When followMouse is off, publish the static/prop tilt value
        if (!followMouse && publishRef?.current) {
          publishRef.current(uniformsRef.current.iTilt.value);
        }
        try {
...
/**
 * SideRaysProvider — wraps the page so that BorderGlow / SpotlightCard
 * can subscribe to the live tilt value published by SideRays.
 *
 * Usage in App.jsx:
 *   const { Provider, publishRef, contextValue } = useSideRaysProvider()
 *   <Provider value={contextValue}> ... <SideRays publishRef={publishRef} /> ... </Provider>
 */
export { SideRaysContext };
